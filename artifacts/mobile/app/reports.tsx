    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: Platform.OS === 'web' ? 34 : 100 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsGrid}>
          {[
            { label: t('revenue'),   value: `${cur} ${totalSales.toFixed(2)}`,   icon: 'cash-outline' as const,       color: colors.primary },
            { label: t('orders'),    value: String(orderCount),                   icon: 'receipt-outline' as const,    color: colors.info },
            { label: t('profit'),    value: `${cur} ${totalProfit.toFixed(2)}`,   icon: 'trending-up-outline' as const, color: colors.success },
            { label: t('avgOrder'),  value: `${cur} ${avgOrder.toFixed(2)}`,      icon: 'stats-chart-outline' as const, color: '#8B5CF6' },
            { label: t('discounts'), value: `${cur} ${totalDiscount.toFixed(2)}`, icon: 'pricetag-outline' as const,   color: colors.warning },
